    ...
    <div class="main h-full grid border-t border-t-gray-300 relative">
        ...
        <div class="relative p-4 w-full max-w-md max-h-full m-auto">
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
              ...
              <button
                @click="cameraStore.terminateNewSharing" type="button" class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="authentication-modal">
                <svg
                  class="w-3 h-3"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="shared_panel h-full flex flex-col w-56 border-r border-r-gray-300" :class="{ hidden: isSidePanel.on }">
        ...
        <ul class="flex justify-evenly text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400">
          <li @click="cameraStore.toggle_active_shared_tab('sharedFiles')" class="border-r flex-1 cursor-pointer" :class="{'activeSharedTab': active_shared_tab == 'sharedFiles', 'inactiveSharedTab': active_shared_tab != 'sharedFiles'}">
            <span class="w-full inline-block p-4">Shared files</span>
          </li>
          <li @click="cameraStore.toggle_active_shared_tab('pinned')" class="flex-1 cursor-pointer" :class="{'activeSharedTab': active_shared_tab == 'pinned', 'inactiveSharedTab': active_shared_tab != 'pinned'}">
            <span class="w-full inline-block p-4">Pinned</span>
          </li>
        </ul>
      ...
      <div class="body h-full relative">
        ...
        <div class="messages absolute top-0 bottom-0 w-full overflow-hidden">
          <video id="remoteVideo" class="video w-full h-full object-cover" autoplay muted>
            <source src="" type="video/mp4" />
            Your browser does not support the video tag.
          </video>
        </div>

        <div class="messages z-30 absolute bottom-16 left-[70%] right-4 h-36 overflow-hidden rounded-lg border border-slate-400">
          <div class="font-bold flex absolute top-0 w-full justify-center bg-black bg-opacity-25">
            You
          </div>
        ...
        <div class="absolute bottom-0 w-full">
          <div class="messageContainer h-80 min-w-1/4 w-[26rem] bg-transparent relative overflow-hidden overflow-y-auto scrollbar-hide">
            <div class="space-y-4 px-4 py-2 items-end">
              <spaceMessage v-for="(message, index) in messages" :key="index" :message="message.value" :sender="message.sender"/>
            </div>
          </div>
      ...
      <!-- Aside panel -->
      <div
        class="side_panel border-l h-full relative border-l-gray-300 w-64"
        :class="{ hidden: !isSidePanel.on }"
      >
...
let messages = computed(() => cameraStore.messages)
...
onMounted(async () => {
  ...
  remoteVideo.srcObject = remoteStreams.value

  console.log("shared pins are ", loadedSpace.value.pins)
...
let attachToggleListner = () => {
  ...
  if (active_shared_tab.value == 'sharedFiles' && shared_panel_tabs != null)
    shared_panel_tabs[0].dispatchEvent(new Event('click'))
