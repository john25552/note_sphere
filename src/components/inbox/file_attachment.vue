<template>
    <div class="flex items-start gap-2.5" :class="{'justify-end mr-3': props.message.sender == 'self'}">
        <button v-if="props.message.sender == 'self'" id="dropdownMenuIconButton" data-dropdown-toggle="dropdownDots" data-dropdown-placement="bottom-start" class="inline-flex self-center items-center p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800 dark:focus:ring-gray-600" type="button">
            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 4 15">
                <path d="M3.5 1.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Zm0 6.041a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Zm0 5.959a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z"/>
            </svg>
        </button>
        <!-- TODO -->
        <img v-if="props.message.sender != 'self'" class="w-8 h-8 rounded-full" src="" :alt="props.message.sender">
        <div class="flex flex-col gap-1">
            <div class="flex flex-col w-full max-w-[326px] leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-s-xl rounded-ee-xl dark:bg-gray-700">
                <div class="flex items-center justify-between space-x-2 rtl:space-x-reverse">
                    <span v-if="props.message.sender !== 'self'" class="text-sm font-semibold text-gray-900 dark:text-white">{{ props.message.sender }}</span>
                    <span v-else></span>
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ props.message.timeSent }}</span>
                </div>
