        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form action="#">
                    ...
                    <div class="flex items-center justify-center w-full col-span-2">
                        <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p class="mb-2 text-sm text-gray-500 dark:text-gray-400"><span class="font-semibold">Click to upload</span> or drag and drop profile photo</p>
                ...
                <div class="flex items-center justify-between space-x-4 w-full">
                    <button @click="saveAccount" type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Save
                    </button>
...
<script setup lang="ts">
    ...
    let userStore = useAccountStore()
    let currentEmail = computed(() => userStore.user?.email_address)
    let currentName = computed(() => {
        return {
            firstName: userStore.user?.first_name as string,
            secondName: userStore.user?.second_name as string
        }
    })

    let email = ref(`${currentEmail.value}`)
    let firstName = ref('')
    let secondName = ref('')

    onMounted(() => {
        if (currentName.value.firstName?.length > 0) {
            firstName.value = currentName.value.firstName
            secondName.value = currentName.value.secondName
        }
    })
